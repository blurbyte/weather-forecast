...
export function getFiveDayForecast(cityId) {
  return fetch(`https://api.openweathermap.org/data/2.5/forecast?id=${cityId}&units=metric&APPID=${API_KEY}`)
    .then(res => res.json())
...
export function getCities(search) {
  ...
  return fetch(
    `https://api.openweathermap.org/data/2.5/find?q=${encodedSearch}&mode=json&cnt=8&type=like&sort=population&APPID=${API_KEY}`
  )
    .then(res => res.json())
